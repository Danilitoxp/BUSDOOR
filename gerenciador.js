...
document.addEventListener("DOMContentLoaded", () => {
  ...
  async function addFornecedor(fornecedor) {
    ...
    async function updateFornecedor(id, fornecedor) {
      try {
        ...
        await updateDoc(fornecedorRef, fornecedor);
        console.log("Fornecedor atualizado com ID: ", id);
      } catch (error) {
        console.error("Erro ao atualizar fornecedor: ", error.message);
        // Aqui você pode exibir uma mensagem de erro mais amigável ao usuário
      }
    }
  ...
  elements.formFornecedor.addEventListener("submit", async (e) => {
    ...
    if (id) {
      ...
      // Garante que 'pagamentos' é um array ou um array vazio
      const pagamentos = fornecedorAtual && fornecedorAtual.pagamentos ? fornecedorAtual.pagamentos : [];
      
      // Atualiza o fornecedor com os dados novos, mantendo a lista de pagamentos existente
      await updateFornecedor(id, { ...fornecedor, pagamentos });
    } else {
